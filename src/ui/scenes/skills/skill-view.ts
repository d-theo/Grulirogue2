...
export class SkillView extends Phaser.GameObjects.Container {
  ...
  inputs(args: { viewW: number; viewH: number; config: any; action: 'useSkill' | 'pickSkill' }) {
    ...
    if (action === 'pickSkill') {
      ...
      this.add(
        this.scene.add
          .text(viewW / 2, y, 'Congratulation, you level up ! Choose a specialization wisely', SkillFont)
          .setOrigin(0.5, 0)
      );
    }
    if (action === 'useSkill') {
    ...
    for (const skill of this.config) {
      ...
      const skillLine = this.scene.add
        .text(x, y, txt, Object.assign({}, SkillFont, { wordWrap: { width: 300 } }))
        .setOrigin(0);
      this.add(skillLine);
