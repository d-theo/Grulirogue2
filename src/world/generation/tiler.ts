...
export function tilemapper(config: TilerConfig) {
  ...
  function tile(map: MapGraph) {
    const tilemap = Array(config.height)
      .fill(Terrain.Void)
      .map(() => Array(config.width).fill(Terrain.Void));
  ...
  function tileLayers(map: MapGraph, tilemap1) {
    const rooms = map.rooms;
    const tilemap2 = Array(config.height)
      .fill(Terrain.Void)
      .map(() => Array(config.width).fill(Terrain.Transparent));
