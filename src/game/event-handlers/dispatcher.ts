...
export class EventDispatcher {
  ...
  init() {
    gameBus.subscribe(itemRemoved, (event) =>
      new ItemRemovedHandler(this.game.getItems()).handle(event)
    );
    gameBus.subscribe(monsterDead, (event) =>
      new MonsterDeadHandler(this.game.getMonsters()).handle(event)
    );
    gameBus.subscribe(heroGainedXp, (event) =>
      new HeroGainedXPHandler(this.game.getHero()).handle(event)
    );

    gameBus.subscribe(playerMoved, (event) =>
      new SightHasChangedHandler(
        this.game.getHero(),
    ...
    gameBus.subscribe(rogueEvent, (event) =>
      new rogueEventHandler().handle(event)
    );
    gameBus.subscribe(endRogueEvent, (event) =>
      new EndRogueEventHandler().handle(event)
    );
