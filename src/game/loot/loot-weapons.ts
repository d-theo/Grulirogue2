...
export const WeaponOnCarryBonus = [
  {
    effect: Buff2.create(() => Conditions.precision({precisionBonus: 0.1})).setTurns(Infinity).setIsStackable(true),
    name: 'of precision',
    description: 'this weapon is very accurate'
  },
  {
  ...
  },
  {
    ...
    name: 'of agility',
    description: 'this weapon is easy enought to manipulate and allows to dodge better'
  },
]
  ...
  for (const e of enchants) {
    switch (e) {
      ...
      case 'plus_five':
        ...
        break;
      case 'stun':
        w.additionnalEffects.push({effect: Buff2.create(Conditions.stun).setTurns(1), target: 'target', chance: 0.07});
        w.additionalDescription.push('can stun the target');
        w.additionalName.push('Stun');
        w.identified = false;
        break;
      case 'bleed':
        w.additionnalEffects.push({effect: Buff2.create(Conditions.bleed).setTurns(3), target: 'target', chance: 0.1});
        w.additionalDescription.push('inflict bleeding');
        w.additionalName.push('Bleeding');
        w.identified = false;
        break;
      case 'poison':
        w.additionnalEffects.push({effect: Buff2.create(Conditions.poison).setTurns(4), target: 'target', chance: 0.5});
        w.additionalDescription.push('poison the target');
        w.additionalName.push('Poison');
        w.identified = false;
        break;
      case 'shock':
        w.additionnalEffects.push({effect: Buff2.create(Conditions.shock).setTurns(1), target: 'target', chance: 0.5});
        ...
        break;
      case 'cold':
        w.additionnalEffects.push({effect: Buff2.create(Conditions.cold).setTurns(1), target: 'target', chance: 0.5});
        ...
        break;
      case 'weakness':
        w.additionnalEffects.push({
          effect: Buff2.create(() => Conditions.weakness({hpWeaker: 5})).setTurns(10),
          target: 'target',
          chance: 0.1
        });
